...
**关键节点要深挖**：如果某个节点的产物/决策**影响后续所有结果生成**（典型如 `DESIGN.md`、契约文件、配置中枢），别一笔带过——拆清两件事：
- **它由什么组成**（有哪些部分/字段/章节）；
- **作者为什么这么设计**（这样拆解解决了什么问题、做了什么取舍）。
...
4. **核心概念**：最多 **3 个**最关键的。每个都讲**两层**——
   - ① 这个概念**解决什么问题**（一句大白话）；
   - ② 如果它有组成元素（如「设计系统生成器」= Pattern / Style / Colors / Typography / Effects / Avoid / Checklist），**逐个说每个元素解决这个概念的什么子问题**（一元素一行，没组成就略过）。
...
**持续维护用户关注点（最简做法）**：用户后续就某个**已分析过的 skill** 继续追问/深入时，小石直接在该 `memory/<id>.md` 正文末尾追加一行 `- [日期] 用户关心了 <什么>`，并据此更新 frontmatter 的 `deep_memory`。无需新命令，用文件编辑追加即可；改完跑一次 `python3 scripts/memory.py index` 刷新索引。这样用户的关注点会持续沉淀，不会停在分析那一刻。
...
- **核心概念讲两层**：每个核心概念先说它解决什么问题；有组成元素的，再逐个说每个元素解决这个概念的什么子问题。
- **记忆必记三件事**：名称与流程 / 流程与关键节点（结构化存）/ 用户最深记忆。
- **关注点持续追加**：用户后续深聊某个已分析 skill，答完就往它的 `memory/<id>.md` 正文加一行关注点并更新 `deep_memory`，别让记忆停在分析那一刻。
